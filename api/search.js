...
import { attachDistanceAndFilter, mergeAndDedupe } from "../lib/server/events.js";
...
export default async function handler(request, response) {
  ...
  const radius = Math.min(Math.max(parseNumber(request.query.radius) || 25, 1), 100);
  const location = String(request.query.location || "").trim();

  if ((lat === null || lng === null) && !location) {
    return response.status(400).json({ error: "Provide coordinates or a city." });
  }

  try {
    const [storedEvents, ticketmasterEvents] = await Promise.all([
    ...
    const merged = mergeAndDedupe([...storedEvents, ...ticketmasterEvents]);

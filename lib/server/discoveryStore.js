  ...
  cells.forEach((cell, index) => {
    ...
    batch.set(
      references[index],
      {
        ...
        attempts: force ? 0 : Number(existing?.attempts || 0),
      },
      { merge: true },
    );
    queuedCount += 1;
...
export async function loadPendingDiscoveryJobs(db, limit = 3) {
  ...
  return snapshot.docs
    ...
    .filter(
      (job) =>
        !job.retryAfter || new Date(job.retryAfter).getTime() <= Date.now(),
    )
    .sort(
      (first, second) =>
        Number(second.priority || 0) - Number(first.priority || 0) ||
        new Date(second.queuedAt || 0).getTime() - new Date(first.queuedAt || 0).getTime(),
    )

import { absoluteUrl, fetchTextResource, stripHtml } from "./htmlUtils.js";
...
export function parseCalendarLocation(value, source = {}) {
  const location = unescapeCalendarText(value);
  if (!location) {
    return {
  ...
  if (/^(?:usa|us|united states(?: of america)?)$/i.test(parts.at(-1) || "")) {
    parts.pop();
  }

  let state = source.state || null;
  let postalCode = source.postalCode || null;
  const stateAndPostalCode = (parts.at(-1) || "").match(
    /^([a-z]{2})(?:\s+(\d{5}(?:-\d{4})?))?$/i,
  );
  if (stateAndPostalCode) {
    state = stateAndPostalCode[1].toUpperCase();
    postalCode = stateAndPostalCode[2] || postalCode;
    parts.pop();
  }

  let city = source.city || null;
  if (stateAndPostalCode && parts.length > 1) {
    city = parts.pop();
  }

  let venueName = source.venueName || null;
  const firstPartIsVenue = parts.length > 0 && !looksLikeStreetAddress(parts[0]);
  if (firstPartIsVenue && (!venueName || parts[0].toLowerCase() === venueName.toLowerCase())) {
    venueName = parts.shift();
  }

  return {
    venueName: venueName || source.name,
    address: parts.join(", ") || source.address || null,
    city,
    state,
    postalCode,
  };
...
function feedEvent(source, values, index) {
  ...
  const locationDetails = parseCalendarLocation(location, source);
...
export async function fetchICalendarEvents(source) {
  const resource = await fetchTextResource(source.url, {
    etag: source.httpEtag,
    lastModified: source.httpLastModified,
  });
  const events = resource.notModified ? [] : parseICalendar(resource.text, source);
  events.collectionStatus = resource;
  return events;
}
...
export async function fetchRssEvents(source) {
  ...
  const events = resource.notModified ? [] : parseEventRss(resource.text, source);
  events.collectionStatus = resource;
  return events;
}

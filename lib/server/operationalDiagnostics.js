...
export function buildOperationalDiagnostics(
  { sources = [], jobs = [], candidates = [], runs = [], audits = [], searches = [], genreCaches = [], genreQueue = [] },
  now = Date.now(),
) {
  ...
  return {
    ...
    summary: {
      ...
      pendingGenreArtists: genreQueue.length,
    },
    sources: sourceRows,
    ...
    coverageAreas,
    genreImpact,
    genreEnrichmentQueue: [...genreQueue].sort(
      (first, second) => String(first.priorityStartTime || "").localeCompare(String(second.priorityStartTime || "")),
    ),
  };
}

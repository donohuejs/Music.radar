...
export function parseMediaDataUrl(value) {
  ...
  if (!bytes.length || bytes.length > MAX_MEDIA_BYTES) {
    throw new Error("Poster image must be smaller than 2.5 MB after compression.");
  }
...
export function storageObject(contentType, id, prefix = "media-leads") {
  const extension = contentType === "image/png" ? "png" : contentType === "image/webp" ? "webp" : "jpg";
  return {
    path: `${prefix}/${id}/poster.${extension}`,
  };
}

export async function signedAssetUrl(bucket, path, { expiresInMs = 15 * 60 * 1000 } = {}) {
...
export async function attachSignedAssetUrls(bucket, candidates, options) {
  if (!bucket) return candidates;
  return Promise.all(candidates.map(async (candidate) => {
    if (!candidate.assetStoragePath) return candidate;
    try {
      return {
        ...candidate,
        assetUrl: await signedAssetUrl(bucket, candidate.assetStoragePath, options),
      };
    } catch (error) {
      console.warn(`Could not sign media lead ${candidate.id || candidate.assetStoragePath}:`, error.message);
      return { ...candidate, assetUrl: null };
    }
  }));
}

...
import { eventDetailUrl, isEventDetailPath, isEventListingPath } from "./eventLinks.js";
...
const PAGE_SIGNALS = /\b(event|events|happenings|shows?|calendar|concert|concerts|music|festival|festivals|entertainment|lineup|schedule|what'?s on|live)\b/i;
...
export function overpassQuery({ latitude, longitude, radiusMiles = 25 }) {
  ...
  const venueAmenities = "bar|pub|biergarten|nightclub|theatre|arts_centre|community_centre|music_venue|concert_hall|events_venue|food_court|marketplace|social_centre";
  return `[out:json][timeout:25];(
    ...
    nwr(${around})["website"]["tourism"="information"]["information"~"^(office|visitor_centre)$"];
    nwr(${around})["contact:website"]["tourism"="information"]["information"~"^(office|visitor_centre)$"];
    nwr(${around})["website"]["shop"="mall"];
    nwr(${around})["contact:website"]["shop"="mall"];
    nwr(${around})["website"]["office"~"^(tourism|association)$"];
    nwr(${around})["contact:website"]["office"~"^(tourism|association)$"];
...
export function detectPageSource(html, pageUrl) {
  ...
  const listingPath = isEventListingPath(pathname);
  const squarespaceListing = /<article\b[^>]*class=["'][^"']*\beventlist-event\b/i.test(normalizedHtml);
  if (!isEventDetailPath(pathname) && squarespaceListing && detailLinks.length) {
    return {
      kind: "calendar", parser: "squarespace", confidence: 0.96,
      detectedProvider: "squarespace", linkedEventCount: detailLinks.length,
      sourceScope: "listing", reusableSource: true,
    };
  }
  if (!isEventDetailPath(pathname) && detailLinks.length >= (listingPath ? 3 : 8)) {
    return {
      kind: "calendar",
  ...
  if (/<script[^>]+type=["']application\/ld\+json["'][^>]*>[\s\S]*?"@type"\s*:\s*(?:"[^"]*Event"|\[[^\]]*"[^"]*Event")/i.test(normalizedHtml)) {
    return {
      ...
      parser: "json-ld",
      confidence: 0.95,
      sourceScope: "single-event",
      reusableSource: false,
    };
  }
...
export function mergeOrganizationSeeds(...groups) {
  const unique = new Map();
  for (const organization of groups.flat()) {
    const key = String(organization?.url || "").replace(/\/$/, "").toLowerCase();
    if (!key) continue;
    const current = unique.get(key);
    if (!current || Number(organization.priority || 0) > Number(current.priority || 0)) {
      unique.set(key, organization);
    }
  }
  return [...unique.values()].sort((first, second) => Number(second.priority || 0) - Number(first.priority || 0));
}

export function eventDetailLinks(html, pageUrl) {
  return [...new Set(extractLinks(html, pageUrl)
    .map((link) => eventDetailUrl(link.url, pageUrl)).filter(Boolean))];
}
  ...
  return unique
    .sort((first, second) => {
      const score = (url) => {
        ...
        return isEventListingPath(path) ? 2 : isEventDetailPath(path) ? 1 : 0;
      };
...
export async function inspectOrganization(
  ...
  const listings = candidates
    .filter((candidate) => ["json-ld-listing", "squarespace"].includes(candidate.parser))
    .sort(
      (first, second) =>
  ...
  return candidates.filter(
    (candidate) =>
      ...
      !["json-ld", "json-ld-listing", "squarespace"].includes(candidate.parser),
  );
}
...
export async function discoverLocationSourceBatch(
  ...
  let organizationDiscoveryError = null;
  let organizations = suppliedOrganizations;
  if (!Array.isArray(organizations) || !organizations.length) {
    let nearbyOrganizations = [];
    try {
      nearbyOrganizations = await discoverNearbyOrganizations(location, {
        timeoutMs: Math.min(24000, Math.max(deadline - Date.now(), 1000)),
      });
    } catch (error) {
      organizationDiscoveryError = error.message;
      if (!seedOrganizations.length) throw error;
    }
    organizations = mergeOrganizationSeeds(
      nearbyOrganizations,
      seedOrganizations,
    ).slice(0, 100);
  }
